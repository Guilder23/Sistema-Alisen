...
function renderizarItemsProforma() {
    ...
    if (proformaItems.length === 0) {
        body.innerHTML = `
            <tr>
                <td colspan="9" class="text-center text-muted py-4">No hay productos agregados.</td>
            </tr>`;
    ...
    fetch(PROFORMAS_EDITAR_URLS.obtenerProforma)
        ...
        .then((data) => {
            ...
            proformaItems = data.items.map((item) => ({
                ...
                precio_unitario: item.modalidad === 'caja' ? item.precio_unidad : item.precio_unitario,
                precio_unidad: item.precio_unidad,
                precio_mayor: item.precio_mayor,
                precio_caja: item.precio_caja,
                precio_unidad_oferta: item.precio_unidad_oferta,
                en_oferta: item.en_oferta,
                unidades_por_caja: item.unidades_por_caja,
                unidades_por_mayor: item.unidades_por_mayor,
                cantidad: item.cantidad,
                modalidad: item.modalidad === 'caja' ? 'unidad' : item.modalidad,
